...
class GameFunctionsService {
    ...
    private fun Iterable<String>.toPhotoCharacters(): List<PhotoCharacter> =
        this.map { PhotoCharacter.valueOf(it.replaceFirstChar { it.titlecase() }) }
    ...
    = toPhotoCharacters().groupBy { it.hairType }.values
        .map { it.groupBy { it.accessories == null || Accessory.Hat !in it.accessories }.values.flatten() }
        .flatten()
